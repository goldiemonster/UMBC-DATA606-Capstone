...
[NOAA nClimGrid-Daily](https://www.ncei.noaa.gov/products/land-based-station/nclimgrid-daily) — NCEI's daily
gridded climate product (daily max/min/average temperature and precipitation, ~5 km/0.0417° resolution,
contiguous U.S., 1951–present), distributed as monthly netCDF files in the public S3 bucket
`noaa-nclimgrid-daily-pds` (no authentication required). Used to test research question 3 (whether
deterioration risk varies by state/region in a way that suggests climate effects).

Pulled for the same **2020–2025** period and 7-state footprint as the bridge inspection data. Each bridge is
matched to its nearest nClimGrid-Daily grid cell using its decoded `lat`/`lon`, and daily values are
aggregated to one row per bridge per year in `data/processed/weather_annual_by_bridge.csv.gz` (built by
[`scripts/fetch_nclimgrid_weather.py`](../scripts/fetch_nclimgrid_weather.py)):
...
| `prcp_total_mm` | float | Annual total precipitation | mm, ≥ 0 |
| `freeze_thaw_days` | int | Count of days where `tmax > 0°C` and `tmin < 0°C` | 0–365 |

Joins onto the bridge dataset via `STRUCTURE_NUMBER_008` + `YEAR`. Raw netCDF grids (~4.6 GB across 72 monthly
files) are downloaded and discarded by the script; only this small aggregated file is stored in the repo.
...
Also `tmax_mean_c`, `tmin_mean_c`, `tavg_mean_c`, `prcp_total_mm`, and `freeze_thaw_days` from
`weather_annual_by_bridge.csv.gz` (joined on `STRUCTURE_NUMBER_008` + `YEAR`), as candidate climate features
